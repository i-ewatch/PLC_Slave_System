...
namespace PLC_Slave_System.Protocols
{
    ...
    {
        public override void ReadData(IModbusMaster master, TCPComponent tCPComponent)
        {
            try
            {
                int Index = 0;
                                ...
                                foreach (bool item in value)
                                {
                                    if (ColisDatas.Count > 0)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                Address = Convert.ToInt32(Pointitem.ReadAddress + Index),
                                            };
            ...
            {
                if (tCPComponent.BackupIndex >= IPSetting.Location.Length - 1)
                {
                    tCPComponent.BackupIndex = 0;
                }
                else
                {
                    if (tCPComponent.BackupIndex < IPSetting.Location.Length - 1)
                    {
                        tCPComponent.BackupIndex++;
                    }
                }
                ConnectionFlag = false;
                Log.Error(ex, $"設備通訊失敗 IP : {IPSetting.Location[LocationIndex]}，Port : {IPSetting.port[LocationIndex]}");
            }
        }
    }
}
